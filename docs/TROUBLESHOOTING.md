...
|---------|---------|---------------|------------|
| Frontend (Next.js) | `pnpm dev` fails with Node version warning | Run `node --version` and confirm it is 20.19+ | Align with repository Node version by running `nvm use` or reinstalling via fnm/Volta, then retry `pnpm install` |
