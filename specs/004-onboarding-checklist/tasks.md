...
- [ ] T024 [P] [US2] Create Playwright scenario verifying template selection primes the canvas + instant backtest, asserts that telemetry records `template_selected` + `backtest_success` events, and that the empty-template state renders the alternate CTA + disabled completion in `/Users/pierrickmartino/Developer/blockbuilders.tech.speckit/apps/frontend/tests/e2e/template-prime.spec.ts`.
...
- [ ] T034 Build Supabase views + Datadog dashboards for SC-01/02/03 using onboarding telemetry in `/Users/pierrickmartino/Developer/blockbuilders.tech.speckit/configs/datadog/onboarding-funnel.json`, documenting KPI queries, the ≤1 s SLA evidence, and the SC-02 (first backtest ≤15 min) + SC-03 (≥70% template success) derivations in `/Users/pierrickmartino/Developer/blockbuilders.tech.speckit/docs/qa/onboarding-checklist.md`.
