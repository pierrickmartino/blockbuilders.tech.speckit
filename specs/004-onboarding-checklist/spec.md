...
1. **Given** CI runs `/scripts/ci/verify-sc-metrics.sh`, **When** SC metrics exceed thresholds, **Then** the job fails, attaches evidence to `/docs/qa/onboarding-checklist.md`, and links the regression to a remediation task.
...
- Template library may be temporarily empty for certain roles; when zero templates are returned the checklist must replace the step with an "Explore strategies" CTA that links to `/docs/templates/strategy-library.md`, disable completion, and show copy explaining why templates are unavailable.
